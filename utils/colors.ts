// src/utils/colors.ts
const colors = {
  primary: "#0b839b", // Neon Cyan
  secondary: "#FF4500", // Neon Orange
  background: "#131344FF", // Deep Space Blue
  panelBackground: "#1A1A3D", // Slightly lighter for panels
  textPrimary: "#E0E0FF", // Soft White
  textSecondary: "#A0A0C0", // Muted Silver Blue
  border: "#4848B3FF", // Space Gray with a hint of blue
  transparentBackground: "rgba(26, 26, 61, 0.5)", // Semi-transparent space panel
  iosTransparentBackground: "rgba(38, 38, 78, 1)", // iOS semi-transparent space panel
  successGradient: ["#25d365", "#056B42"], // Bright Green Gradient
  buttonGreen: "#23A188FF", // Bright Green
  error: "#FF3860", // Vibrant Red for errors
  redButton: "#b6213f", // Bright Red,
  warning: "#FFDD33", // Bright Yellow
  info: "#4CB5F5", // Electric Blue
  hudBlue: "#1F4068", // Dark Blue for HUD elements
  glowEffect: "rgba(15, 240, 252, 0.8)", // Glow for active buttons/elements
  shadow: "rgba(0, 0, 0, 0.6)", // Subtle shadow for depth
  hoverHighlight: "rgba(255, 255, 255, 0.1)", // Soft glow for hover
  disabled: "#3A3A3A", // Muted gray for disabled state
  disabledBorder: "#444444FF", // Subtle border for disabled buttons
  disabledText: "#555555", // Muted gray for disabled text
  disabledBackground: "#282828", // Muted gray for disabled backgrounds
  lockedBackground: "#1a1a1a1b", // Dark gray for locked content
  disabledIcon: "#7e7c7c95",
};

export default colors;
